...
namespace HostBot
{
	...
	BotGame::BotGame(uint id, std::string& gameName, std::string& hostName, MapCfg::Pointer cfg, BotGame::MapSpeed_t spd, BotGame::MapVis_t vis, BotGame::MapObs_t obs, uint flag): _mapCfg(cfg)
	{
		...
		if((_mapCfg->GetFlags() & 0x20) == 0)
			...
			_gameType = 1;
		Common::Stream st;
		st<<_platform<<_version<<_id<<Common::GetTime()<<_gameName<<(byte)0;
		...
		else
		{
			...
			_randomSeed = Common::GetTicks();
			_totalSlots = (uint)(_obs > MAPOBS_ONDEFEAT) ? 12 : _mapCfg->GetRawNumPlayers();
		}
		_slots = _mapCfg->GetSlots();
		if(_mapCfg->GetRawNumPlayers() < _totalSlots)
		{
			for(uint i = _mapCfg->GetRawNumPlayers(); i < _totalSlots; ++ i)
			{
		...
		_freeSlots = 0x1000;
		ReCalcFreeSlots();
		st<<(byte)0<<_totalSlots<<_gameType<<(uint)1;
		_gameInfo = st;
		_startTime = Common::GetTime();
		_stage = STAGE_PREPARE;
		Common::Stream tosend;
	...
	void BotGame::SendUDPInfo( uint ip, ushort port )
	{
		if(_stage != STAGE_PREPARE)
			return;
		Common::Stream tosend(_gameInfo);
		tosend<<_freeSlots<<(uint)0<<(ushort)botCfg.TcpPort();
	...
	bool BotGame::ApplyNewSlot( Player* ptr, byte newId )
	{
		...
		{
			i = newId;
			if(_slots[i].GetSlotStatus() == GameSlot::SLOTSTATUS_OCCUPIED || _slots[i].GetSlotStatus() == GameSlot::SLOTSTATUS_CLOSED)
				return false;
		}
		else
		{
	...
	void BotGame::Kick( uint s, uint reason )
	{
		...
		else
		{
			...
			if(end)
			{
				BotGameSet::GetSingleton().RemoveGame(_id);
			}
		}
	}
	...
	bool BotGame::SetMapProgress( uint id, uint prog )
	{
		...
		if(_players[id - 1] == NULL)
			return true;
		uint sid = _players[id - 1]->GetSlotID();
	...
	{
		used = _totalSlots - _freeSlots;
		total = _totalSlots;
	}

	void BotGame::SendToAll( ushort op, Common::Stream& st, byte except )
	{
	...
	void BotGame::SendTo( ushort op, Common::Stream& st, std::vector<byte>& toid )
	{
		std::vector<byte> sbuf;
		size_t len = st.size();
		sbuf.resize(4 + len);
		...
		if(_gameType == 1)
		{
			...
			do
			{
				...
				if(_slots[i].GetSlotStatus() == GameSlot::SLOTSTATUS_OPEN && _slots[i].GetTeam() == v)
				{
					...
					if(opid == 0 || opid > 12 || _players[opid - 1] == 0)
						return;
	...
	void BotGame::ProcessCommand( byte oid, std::string cmd, std::string arg )
	{
		...
		{
			if(_stage != STAGE_PREPARE || oid == 0 || oid > 12 || _players[oid - 1] == NULL)
				return;
	...
	void BotGame::SendAllActions(uint ptime)
	{
		...
		{
			std::vector<byte>& action = _actions.front();
			if(st.size() + action.size() > 1456)
			{
				*(ushort *)(byte *)st = 0;
	...
	void BotGame::CheckPlayersLag()
	{
		...
		if(_stage == STAGE_LAGGING)
		{
			...
			for(uint i = 0; i < 12; ++ i)
			{
				if(_players[i] != NULL && _players[i]->GetLag() > 0)
				{
					...
					else if(Common::GetTicks() - _players[i]->GetLag() >= 35 * 1000)
					{
		...
		else
		{
			...
			for(uint i = 0; i < 12; ++ i)
			{
				...
				{
					_players[i]->SetLag(Common::GetTicks());
					lag = true;
				}
			}
			if(lag)
	...
	void BotGame::SendStartLag()
	{
		...
		for(uint i = 0; i < 12; ++ i)
		{
			if(_players[i] != NULL)
			{
				...
				{
					st<<(byte)_players[i]->GetPID()<<(Common::GetTicks() - _players[i]->GetLag());
					++ c;
				}
			}
		}
	...
	{
		SendChatTo(id, "Welcome to Garena Autobot game!");

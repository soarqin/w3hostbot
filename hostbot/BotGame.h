...
{
	class BotGame:
		public Common::SharedClass<BotGame>
	{
	public:
		enum Stage_t
		{
		...
		void BuildMapCheck(Common::Stream& st);
		void SendToAll(ushort op, Common::Stream& st, byte = 12);
		void SendToAll(ushort op, byte = 12);
		void SendToAll(const byte * buf, size_t len, byte = 12);
		void SendTo(ushort, Common::Stream&, std::vector<byte>&);
		...
		inline uint GetFreeSlots() {return _freeSlots;}

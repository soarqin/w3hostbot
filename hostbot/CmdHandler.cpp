...
namespace HostBot
{
	...
	bool CmdHandler::CmdGameList( BotReceiver& rcver, StrList_t& params )
	{
		...
		for(size_t i = 0; i < c; ++ i)
		{
			...
			uint avail = total - glist[i]->GetFreeSlots();
			sprintf(row, "ID: %d  Name: %s  Map: %s  Players: %d/%d  Stage: %s\r\n", glist[i]->GetID(), glist[i]->GetName().c_str(), glist[i]->GetMap().c_str(), avail, total, glist[i]->GetStageName().c_str());
			rcver.Send(row);
		}
		return true;
	}
	...
	bool CmdHandler::CmdCancel( BotReceiver& rcver, StrList_t& params )
	{
		...
		BotGameSet::GetSingleton().RemoveGame(ptr->GetID());

...
namespace HostBot
{
	...
	void BotReceiver::Send( ushort op, Common::Stream& buf )
	{
	...
	void BotReceiver::ProcessWC3()
	{
		...
		size_t size;
		ushort psize;
		while((size = _buffer.size()) >= 4 && (psize = *(ushort *)(buf + 2)) <= size)
		{
		    Common::Stream st(buf + 4, psize - 4);
			Interface::PluginManager::GetSingleton().Process(*this, *(ushort *)&buf[0], st);
			if(psize < size)

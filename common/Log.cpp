...
#ifdef _WIN32
			m_fout = CreateFile(_T("CONOUT$"), GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL/* | FILE_FLAG_WRITE_THROUGH*/, NULL);
#else
			freopen_s(&m_fout, "CONOUT$","w+t", stdout);
...
#else
			_tfopen_s(&m_fout, m_filename.c_str(), _T("a+t"));
			setvbuf(m_fout, NULL, _IONBF, 0);
#endif
		}
	}
